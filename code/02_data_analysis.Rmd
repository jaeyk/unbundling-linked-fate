...
p1 <- corr_plot(df %>% filter(race == "Asian")) + ggtitle("Asian Americans")
p2 <- corr_plot(df %>% filter(race == "Black")) + ggtitle("Blacks")
p3 <- corr_plot(df %>% filter(race == "White")) + ggtitle("Whites")
p4 <- corr_plot(df %>% filter(race == "Latinx")) + ggtitle("Latinx")

p1 + p4 + p3 + p2

ggsave("/home/jae/validating-two-linked-fates/outputs/density_plot.png")
```
