...
df_scaled <- df %>%
  ...
  mutate(traditional = scales::rescale(traditional, to = c(-1,1)),
         alternative = scales::rescale(alternative, to = c(-1,1)))

table2 <- df_scaled %>%
  pivot_longer(cols = c(traditional, alternative), 
               names_to = "type", 
               values_to = "value") %>%
  group_by(race, type) %>%
  summarise(
...
```{r}
# recode
df$likely_to_vote <- ((max(df$likely_to_vote) + 1) - df$likely_to_vote)

df$re_elect_trump <- ((max(df$re_elect_trump) + 1) - df$re_elect_trump)
...
ggsave(here("outputs", "predictions_policy.png"))
```