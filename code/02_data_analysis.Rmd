...
df <- df %>%
    mutate(
           ...
           White = factor(ifelse(str_detect(race, "White"), 1, 0)),
           
           Black = factor(ifelse(str_detect(race, "Black"), 1, 0)),

           Asian = factor(ifelse(str_detect(race, "Asian"), 1, 0)),

           Latinx = factor(ifelse(str_detect(race, "Latin"), 1, 0)),
...
df_binded <- bind_rows(
    ...
    # Unweighted group 
          df %>% 
    mutate(fate_progress = -(linked_fate - linked_progress), 
           type = "Unweighted") %>%
    mutate(fate_hurt = -(linked_fate - linked_hurt), 
           type = "Unweighted"),

    # Weighted group 
          df %>%
    mutate(fate_progress = -(wt_linked_fate - wt_linked_progress), 
           type = "Weighted") %>%
    mutate(fate_hurt = -(wt_linked_fate - wt_linked_hurt), 
           type = "Weighted")) %>%
  
    # Row bind then pivot longer 
    pivot_longer(cols = c("fate_progress", "fate_hurt"),
                 names_to = "DV",
...
fit1 <- df_glanced %>%
    mutate(DV = recode(DV, 
           ...
           'fate_hurt' = 'Hurt - Fate')) %>% 
    ggplot(aes(x = fct_reorder(type, r.squared), 
               y = r.squared)) +
        geom_point() +
        coord_flip() +
        labs(x = "", 
...
df_tidied <- df_tidied %>%
    ...
    mutate(term = str_replace(term, "1", "")) %>%
    mutate(DV = recode(DV, 
           'fate_progress' = 'Progress - Fate',
           'fate_hurt' = 'Hurt - Fate')) 
