...
df <- df %>%
    mutate(
           ...
           Democrat = ifelse(party_id == 1, 1, 0),
           Republican = ifelse(party_id == 2, 1, 0),
           for_born = ifelse(for_born %in% c(1, 2), for_born, NA), 
           for_born = ifelse(for_born == 2, 1, 0),
...
fit1 <- df_glanced %>%
    mutate(DV = recode(DV, 
           ...
           'fate_hurt' = 'Hurt - Fate')) %>% 
    ggplot(aes(x = fct_reorder(race, r.squared), 
               y = r.squared, 
               col = type)) +
        geom_point() +
        coord_flip() +
        labs(x = "", title = "R-squared",
             col = "Type") +
        facet_wrap(~DV) +
        geom_hline(yintercept = 0, 
                   linetype = "dashed") 
...
fit2 <- df_glanced %>%
    ...
    ggplot(aes(x = fct_reorder(race, r.squared), 
        ...
        geom_point() +
        coord_flip() +
        labs(x = "", title = "Adjusted R-squared",
             col = "Type") +
        facet_wrap(~DV) +
        geom_hline(yintercept = 0, 
                   linetype = "dashed") 
